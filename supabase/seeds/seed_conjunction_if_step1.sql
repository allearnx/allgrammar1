...
BEGIN
  ...
  q := jsonb_build_array(
    ...
    -- ═══════════════════════════════════════
    jsonb_build_object('number',11,'question',E'다음 밑줄 친 if의 의미로 알맞은 것을 고르시오.\n\nI don''t know <u>if</u> he is honest.','options',jsonb_build_array('~하면','~인지 아닌지','~때문에','~임에도 불구하고','~할 때'),'answer','2'),
    jsonb_build_object('number',12,'question',E'다음 밑줄 친 if의 의미로 알맞은 것을 고르시오.\n\n<u>If</u> it rains tomorrow, we will stay home.','options',jsonb_build_array('~인지 아닌지','~할 때','~하면','~때문에','~임에도 불구하고'),'answer','3'),
    jsonb_build_object('number',13,'question',E'다음 밑줄 친 if의 의미로 알맞은 것을 고르시오.\n\nShe wonders <u>if</u> the store sells fresh bread.','options',jsonb_build_array('~하면','~때문에','~할 때','~인지 아닌지','~임에도 불구하고'),'answer','4'),
    jsonb_build_object('number',14,'question',E'다음 밑줄 친 if의 의미로 알맞은 것을 고르시오.\n\n<u>If</u> you study hard, you will pass the test.','options',jsonb_build_array('~인지 아닌지','~하면','~때문에','~할 때','~임에도 불구하고'),'answer','2'),
    jsonb_build_object('number',15,'question',E'다음 밑줄 친 if의 의미로 알맞은 것을 고르시오.\n\nPlease ask him <u>if</u> he can come to the meeting.','options',jsonb_build_array('~하면','~할 때','~인지 아닌지','~때문에','~임에도 불구하고'),'answer','3'),
    jsonb_build_object('number',16,'question',E'다음 밑줄 친 if의 의미로 알맞은 것을 고르시오.\n\nWe are unsure <u>if</u> the bus has already left.','options',jsonb_build_array('~하면','~인지 아닌지','~할 때','~때문에','~임에도 불구하고'),'answer','2'),
    jsonb_build_object('number',17,'question',E'다음 밑줄 친 if의 의미로 알맞은 것을 고르시오.\n\nYou will feel cold <u>if</u> you don''t wear a coat.','options',jsonb_build_array('~인지 아닌지','~때문에','~하면','~할 때','~임에도 불구하고'),'answer','3'),
    jsonb_build_object('number',18,'question',E'다음 밑줄 친 if의 의미로 알맞은 것을 고르시오.\n\nI want to find out <u>if</u> this recipe works well.','options',jsonb_build_array('~하면','~할 때','~때문에','~임에도 불구하고','~인지 아닌지'),'answer','5'),
    jsonb_build_object('number',19,'question',E'다음 밑줄 친 if의 의미로 알맞은 것을 고르시오.\n\nHe checked <u>if</u> his name was on the list.','options',jsonb_build_array('~인지 아닌지','~하면','~할 때','~때문에','~임에도 불구하고'),'answer','1'),
    jsonb_build_object('number',20,'question',E'다음 밑줄 친 if의 의미로 알맞은 것을 고르시오.\n\n<u>If</u> you are hungry, there is some food in the kitchen.','options',jsonb_build_array('~인지 아닌지','~때문에','~임에도 불구하고','~할 때','~하면'),'answer','5')
  )
  ||
  jsonb_build_array(
    -- ═══════════════════════════════════════
    ...
    -- ═══════════════════════════════════════
    jsonb_build_object('number',21,'question',E'다음 밑줄 친 if의 쓰임이 나머지와 다른 것을 고르시오.','options',jsonb_build_array('I wonder <u>if</u> she passed the exam.','He asked <u>if</u> I wanted some water.','She doesn''t know <u>if</u> the movie is good.','<u>If</u> you press this button, the light turns on.','I''m curious <u>if</u> they arrived safely.'),'answer','4'),
    jsonb_build_object('number',22,'question',E'다음 밑줄 친 if의 쓰임이 나머지와 다른 것을 고르시오.','options',jsonb_build_array('<u>If</u> the weather is nice, let''s go outside.','<u>If</u> you don''t hurry, you''ll be late.','<u>If</u> she calls, tell her I''m busy.','I doubt <u>if</u> this answer is correct.','<u>If</u> he studies more, he will improve.'),'answer','4'),
    jsonb_build_object('number',23,'question',E'다음 밑줄 친 if의 쓰임이 나머지와 다른 것을 고르시오.','options',jsonb_build_array('Do you know <u>if</u> the teacher is here?','I forgot <u>if</u> I locked the front door.','Please check <u>if</u> the oven is off.','We wonder <u>if</u> they sell tickets online.','<u>If</u> it snows, school will be closed.'),'answer','5'),
    jsonb_build_object('number',24,'question',E'다음 밑줄 친 if의 쓰임이 나머지와 다른 것을 고르시오.','options',jsonb_build_array('<u>If</u> you eat too much candy, you''ll get a cavity.','I want to know <u>if</u> the train has left.','<u>If</u> you mix red and blue, you get purple.','<u>If</u> she is free, she will help us.','<u>If</u> it''s cold outside, wear a scarf.'),'answer','2'),
    jsonb_build_object('number',25,'question',E'다음 밑줄 친 if의 쓰임이 나머지와 다른 것을 고르시오.','options',jsonb_build_array('She asked <u>if</u> I could lend her a pencil.','He wonders <u>if</u> the game will be fun.','Tell me <u>if</u> you need any help.','I''m not sure <u>if</u> she likes cats.','Nobody knows <u>if</u> the rumor is true.'),'answer','3'),
    jsonb_build_object('number',26,'question',E'다음 밑줄 친 if의 쓰임이 나머지와 다른 것을 고르시오.','options',jsonb_build_array('I wonder <u>if</u> he will call me tonight.','They asked <u>if</u> we wanted to join them.','<u>If</u> you practice hard, you''ll win the match.','She doesn''t know <u>if</u> the store is still open.','He doubts <u>if</u> the story is real.'),'answer','3'),
    jsonb_build_object('number',27,'question',E'다음 밑줄 친 if의 쓰임이 나머지와 다른 것을 고르시오.','options',jsonb_build_array('<u>If</u> you run fast, you can catch the bus.','<u>If</u> you water the plants, they''ll grow well.','<u>If</u> he apologizes, I will forgive him.','<u>If</u> you touch that wire, you''ll get a shock.','Can you check <u>if</u> the printer is working?'),'answer','5'),
    jsonb_build_object('number',28,'question',E'다음 밑줄 친 if의 쓰임이 나머지와 다른 것을 고르시오.','options',jsonb_build_array('I''m curious <u>if</u> the new student is friendly.','He can''t decide <u>if</u> he should go or stay.','She noticed <u>if</u> anyone was watching her.','We''re unsure <u>if</u> the answer is right.','<u>If</u> you leave now, you won''t miss the train.'),'answer','5'),
    jsonb_build_object('number',29,'question',E'다음 밑줄 친 if의 쓰임이 나머지와 다른 것을 고르시오.','options',jsonb_build_array('<u>If</u> the alarm rings, leave the building quickly.','I need to find out <u>if</u> she got my message.','<u>If</u> you don''t eat breakfast, you''ll be tired.','<u>If</u> they finish early, they can play outside.','<u>If</u> you are sick, you should rest at home.'),'answer','2'),
    jsonb_build_object('number',30,'question',E'다음 밑줄 친 if의 쓰임이 나머지와 다른 것을 고르시오.','options',jsonb_build_array('He wondered <u>if</u> the museum was open on Sundays.','I asked <u>if</u> she needed a ride home.','Do you know <u>if</u> they have a pet?','Please find out <u>if</u> the flight is delayed.','<u>If</u> you wash your hands, you''ll stay healthy.'),'answer','5'),
    jsonb_build_object('number',31,'question',E'다음 밑줄 친 if의 쓰임이 나머지와 다른 것을 고르시오.','options',jsonb_build_array('She wonders <u>if</u> the cookies are ready.','<u>If</u> you boil water, it becomes steam.','He doesn''t know <u>if</u> the road is safe.','I doubt <u>if</u> this plan will succeed.','We can''t tell <u>if</u> the fruit is fresh.'),'answer','2'),
    jsonb_build_object('number',32,'question',E'다음 밑줄 친 if의 쓰임이 나머지와 다른 것을 고르시오.','options',jsonb_build_array('<u>If</u> you keep trying, you''ll get better.','<u>If</u> he works hard, he can earn more money.','<u>If</u> you stand in the rain, you''ll get wet.','I want to see <u>if</u> my friend passed the test.','<u>If</u> she listens carefully, she''ll understand.'),'answer','4'),
    jsonb_build_object('number',33,'question',E'다음 밑줄 친 if의 쓰임이 나머지와 다른 것을 고르시오.','options',jsonb_build_array('She remembers <u>if</u> she brought her umbrella.','He forgot <u>if</u> the meeting was at three.','I''m not certain <u>if</u> my answer is correct.','<u>If</u> you break the rule, you''ll be punished.','They wonder <u>if</u> the park is crowded.'),'answer','4'),
    jsonb_build_object('number',34,'question',E'다음 밑줄 친 if의 쓰임이 나머지와 다른 것을 고르시오.','options',jsonb_build_array('I asked the teacher <u>if</u> we had homework.','She questioned <u>if</u> the news was accurate.','He wants to know <u>if</u> the gym is open.','<u>If</u> you drink enough water, you''ll feel better.','We doubt <u>if</u> the rumor is true.'),'answer','4'),
    jsonb_build_object('number',35,'question',E'다음 밑줄 친 if의 쓰임이 나머지와 다른 것을 고르시오.','options',jsonb_build_array('<u>If</u> you save money, you can buy a new phone.','Can you tell me <u>if</u> the bus stops here?','I need to check <u>if</u> my email was sent.','She wonders <u>if</u> her dad will cook tonight.','He asked <u>if</u> I could help him move.'),'answer','1')
  )
  ||
  jsonb_build_array(
    -- ═══════════════════════════════════════
