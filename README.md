...
- **88 migrations**: `supabase/migrations/001_*.sql` ~ `088_*.sql`
...
```bash
npm run test              # vitest 전체 실행 (767 tests)
npx vitest run --watch    # watch mode
npx tsc --noEmit          # type check only
```

테스트 위치: `src/__tests__/` (52 파일, 767 테스트)
