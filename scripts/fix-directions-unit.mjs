...
// 백업: scripts/backups/directions-backup-'+UNIT.slice(0,8)+'-20260828.json
import fs from 'fs';
...
const UNIT = process.argv.find(a=>a.startsWith('--unit='))?.slice(7);
if (!UNIT) { console.error('--unit=<id> 필요'); process.exit(1); }

const hasInstr = (t) =>
  /[가-힣][^\n]*(시오|하세요|쓰세요|고르|쓰시오|완성하|배열하|채우|바꾸어|고쳐|고치|넣으|답하|찾아)/.test(t) ||
  /(것은|문장은|개수는|하나는|것을|말은|말로|단어는|단어를|말을)[^가-힣]{0,3}\?/.test(t) ||
  /complete the (passage|sentence|dialogue)/i.test(t) ||
  /(put the words|choose |fill in the blank|rearrange|make a sentence|answer the question|rewrite the|as shown in the example)/i.test(t) ||
  /^(which|what|how|when|find|select)\b[^\n]*\?/im.test(t);
...
function classify(q) {
  ...
  if (t.trimStart().startsWith('[보기]')) {
    if (/→/.test(t.split('\n')[0])) return 'SUBJ_보기예시'; // [보기]가 변환 예시인 유형
    return /[가-힣]{2,}/.test(t.split('\n').slice(1).join('\n')) ? 'SUBJ_보기우리말' : 'SUBJ_보기빈칸';
  }
...
const INSTR = {
  ...
  'SUBJ_보기예시': '다음 [보기]와 같이 문장을 바꾸어 쓰시오.',
...
if (sheets.length === 0) { console.error('시트 없음:', sheets.length); process.exit(1); }
fs.mkdirSync('scripts/backups', { recursive: true });
const bak = 'scripts/backups/directions-backup-'+UNIT.slice(0,8)+'-20260828.json';
