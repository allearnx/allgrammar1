 ...
 *   SUPABASE_PROJECT_ID   - 프로젝트 ref (lrzomudeakboasawnvjl)
...
if (!token || !projectId || !dbPassword) {
  ...
  console.log('[db-push] 로컬 개발환경에서는 수동으로 npx supabase db push 실행하세요');
  process.exit(0);
}
...
try {
  ...
  execSync(
    `npx supabase db push --project-ref ${projectId} --password ${dbPassword}`,
    {
      stdio: 'inherit',
  ...
  console.error('[db-push] 마이그레이션 적용 실패! 빌드를 중단합니다.');
  process.exit(1);
}
