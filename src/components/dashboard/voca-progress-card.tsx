...
import { CheckCircle, BookOpen, ClipboardList, CalendarCheck } from 'lucide-react';
...
export function VocaProgressCard({ vocaProgress, submissionStatuses = {} }: Props) {
  ...
  // 오늘 학습한 Day 필터 (KST 기준)
  const todayKST = new Date(new Date().toLocaleString('en-US', { timeZone: 'Asia/Seoul' }));
  const todayStr = format(todayKST, 'yyyy-MM-dd');
  const todayDays = vocaProgress.filter((vp) => {
    if (!vp.updated_at || !vp.day) return false;
    const updatedKST = new Date(new Date(vp.updated_at).toLocaleString('en-US', { timeZone: 'Asia/Seoul' }));
    return format(updatedKST, 'yyyy-MM-dd') === todayStr;
  });
        ...
        {todayDays.length > 0 && (
          <div className="rounded-xl border-2 border-violet-200 bg-violet-50/50 p-4 dark:border-violet-800 dark:bg-violet-950/30">
            ...
            <div className="space-y-2">
              {todayDays.map((vp) => (
                <DayRow key={`today-${vp.day_id}`} vp={vp} submissionStatuses={submissionStatuses} />
              ))}
            </div>
          </div>
        )}
...
function DayRow({ vp, submissionStatuses }: { vp: VocaProgressRow; submissionStatuses: Record<string, string> }) {
  const day = vp.day!;
  ...
  return (
    <div className={`rounded-lg border p-3 ${progressBorderClass(allDone ? 2 : hasAny ? 1 : 0, 2)}`}>
