...
export function FlowCta({ stage, dayId, dayTitle }: { stage: VocaStage; dayId: string; dayTitle?: string }) {
  ...
  return (
    <div className="flex items-center justify-between rounded-xl p-3.5 md:p-4" style={{ background: 'linear-gradient(120deg, #E8F0FE, #D2E3FC)', border: '1px solid rgba(37,99,235,0.08)' }}>
      <div className="mr-3 min-w-0">
        <div className="text-sm font-semibold" style={{ color: '#1A73E8' }}>{cta.title}</div>
      ...
      <Link
        ...
        style={{ background: '#1A73E8', boxShadow: '0 4px 12px rgba(37,99,235,0.15)' }}
      >
        {stage.label} 시작하기 →
      </Link>
    </div>
