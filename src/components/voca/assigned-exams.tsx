...
export function AssignedExams() {
  ...
  return (
    <div className="rounded-2xl border-2 border-indigo-200 bg-indigo-50/50 p-4 space-y-2">
      <p className="font-bold text-indigo-700">📋 선생님이 낸 시험</p>
      {assignments.map((a) => (
        <div key={a.id} className="flex items-center justify-between gap-2 rounded-xl bg-white px-4 py-3">
          ...
          <button
            onClick={() => start(a)}
            className="shrink-0 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-bold text-white hover:bg-indigo-700"
          >
