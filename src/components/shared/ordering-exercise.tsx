...
function SortableWord({ id, word, index, group }: { id: string; word: string; index: number; group: string }) {
...
export function OrderingExercise({ passage, onComplete, sortableGroup = 'words' }: OrderingExerciseProps) {
        ...
        <DragDropProvider onDragOver={handleDragOver}>
          <div className="flex flex-col gap-2">
            {items.map((word, index) => (
              <SortableWord key={`${word}-${index}`} id={`${word}-${index}`} word={word} index={index} group={sortableGroup} />
            ))}
          </div>
        </DragDropProvider>
      </div>
