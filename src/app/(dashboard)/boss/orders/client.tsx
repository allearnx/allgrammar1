  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
        <StatCard label="총 매출" value={formatKRW(totalRevenue)} sub="결제완료 기준" color="#4285F4" icon={<DollarSign className="h-5 w-5" />} />
        <StatCard label="결제완료" value={paidCount} sub="성공 건수" color="#4285F4" icon={<CheckCircle2 className="h-5 w-5" />} />
        <StatCard label="실패" value={failedCount} sub="결제 실패" color="#9CA3AF" icon={<XCircle className="h-5 w-5" />} />
        <StatCard label="환불" value={refundedCount} sub="환불 처리" color="#8AB4F8" icon={<RotateCcw className="h-5 w-5" />} />
      </div>
