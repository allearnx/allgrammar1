...
export async function POST(request: NextRequest) {
  ...
  const { quizSetId, unitId, score, wrongWords } = await request.json();
  if (!quizSetId || score === undefined) {
  ...
  if (score >= 80 && unitId) {
    // Count total quiz sets for this unit
    const { count: totalSets } = await supabase
      .from('naesin_vocab_quiz_sets')
      .select('*', { count: 'exact', head: true })
      .eq('unit_id', unitId);

    // Count completed sets (best score >= 80%)
