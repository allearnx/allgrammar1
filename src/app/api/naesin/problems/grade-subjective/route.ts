...
export const POST = createApiHandler(
  ...
  async ({ user, body }) => {
      ...
      if (now < userLimit.resetAt) {
        if (userLimit.count >= RATE_LIMIT) {
          return NextResponse.json(
            { error: '시간당 채점 횟수(20회)를 초과했습니다. 잠시 후 다시 시도해주세요.' },
            { status: 429 }
          );
        }
        userLimit.count++;
    ...
    // Check cache
    const cacheKey = `${question}::${studentAnswer.trim().toLowerCase()}`;
    ...
    try {
      const message = await anthropic.messages.create({
        ...
        messages: [
          {
            role: 'user',
            content: `You are an English teacher grading a Korean middle school student's written answer on a grammar exam.
...
Grade the student's answer. Respond in JSON format only:
{
  "score": <0-100 number>,
  "feedback": "<brief feedback in Korean, 2-3 sentences>",
  "correctedAnswer": "<the correct/improved English answer>"
}
