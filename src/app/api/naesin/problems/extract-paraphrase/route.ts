...
- **객관식은 원본에 선다형 선택지가 인쇄된 문제만**: 보기 상자(word bank) 빈칸 채우기나 "(A / B)" 괄호 선택형을 객관식(5지선다)으로 바꾸지 말 것 — 서술형 그대로 유지. word bank는 "[보기] ..." 형태로 question에 포함하고 answer는 정답 단어/형태만
...
- **오류 수정형의 answer는 "고친 형태"만(고친 단어/구 또는 전체 고친 문장).** "틀린단어 / 고친단어" 쌍(예: "be / is")이나 화살표 쌍(예: "boring → bored")으로 만들지 말 것 — 학생은 고친 형태만 입력하므로 쌍이 저장되면 전원 오답. question 끝에 " ※ 고친 부분만 쓰시오." 안내, 정답 후보 여럿이면 acceptedAnswers로
- 중학생 수준에 적합한 난이도
