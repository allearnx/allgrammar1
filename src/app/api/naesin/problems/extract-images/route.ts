...
- **밑줄·강조 보존**: 원본에서 밑줄/강조된 부분은 <u>...</u> 태그로 감싸세요. 특히 "밑줄 친 부분/단어"를 묻는 문항은 그 대상(단어·구·문장)을 반드시 <u>로 표시해야 합니다 — 안 하면 학생이 어디가 밑줄인지 몰라 못 풉니다. 보기 속 특정 단어를 묻는 경우(어휘 동의어 등) 각 보기의 해당 단어도 <u>로 감싸세요. (단, 지문에 ⓐ~ⓔ / (A)~(E) / ㉠~㉤ 기호 마커로 위치가 이미 표시된 경우는 그 마커로 충분 — 추가 <u> 불필요.)
...
- **★그림·도표 의존 문항은 그림 내용을 인라인으로 묘사 (삭제 방지):** "다음 그림을 보고 / 그림과 일치하는 / 사진 속 / 표지판을 보고 / 지도를 보고 / 도표를 참고" 처럼 그림·사진·지도·표지판·도표를 봐야 풀 수 있는 문항은, **당신이 지금 보고 있는 그 이미지의 내용을 question 안에 한국어로 \`(그림: ...)\` 형식으로 묘사**해 넣으세요. 정답을 가르는 핵심 정보(인물의 동작·위치·개수, 표지판 문구, 도표 수치, 화살표 방향 등)를 빠짐없이 적어야 학생이 원본 그림 없이도 풀 수 있습니다.
  - 예: "다음 그림을 보고 빈칸에 알맞은 말을 쓰시오. (그림: 한 소년이 우산을 들고 비 오는 거리를 걷고 있다.) He is ___ in the rain."
  - **인라인 묘사 \`(그림: ...)\`가 없으면 그 문항은 그림 없이 풀 수 없어 저장 시 자동 삭제됩니다 — 반드시 묘사를 넣으세요.**
  - 단, 그림이 장식용(풀이에 불필요)이면 묘사 불필요. 보이지 않는 내용을 지어내지 말고 이미지에 실제로 보이는 것만 묘사.
