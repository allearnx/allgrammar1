...
- **2택 고르기는 2지선다 객관식으로 추출**: "(A / B)" 괄호 선택이나 "□ A □ B" 박스 선택은 괄호·박스를 question에서 빼고 그 자리를 빈칸(______)으로 바꾼 뒤, options에 두 선택지만 넣고 answer는 정답 번호(1~2)
...
- 문제 번호(number)는 원본에 적힌 원래 번호를 그대로 사용. **단, question 텍스트 안에는 "7." 같은 원본 문항 번호를 넣지 말 것** — 하나의 지시문 아래 번호 항목이 이어지는 형식이면 각 문항은 지시문 + 해당 항목 내용만 담을 것 (시트가 번호를 새로 매기므로 본문에 남으면 이중 번호로 어긋남)
- **★각 문항은 학생 화면에 한 문제씩 단독 표시됩니다**: 모든 문항의 question은 그것만 보고 무엇을 해야 하는지 알 수 있어야 합니다. 공통 지시문은 묶음의 모든 문항 question 맨 앞에 각각 복제하세요 — 지시문 없이 문장·빈칸만 남기지 마세요. 기호 지시((?)·(+) 등)만 있는 원본은 뜻을 풀어서("(?)=의문문으로 바꾸기") 지시문으로 쓰세요.
