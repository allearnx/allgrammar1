...
export function getCtaText(stage: VocaStage, dayTitle?: string): { title: string; sub: string } {
  const prefix = dayTitle ? `${dayTitle} ` : '';
  switch (stage.key) {
    case 'flashcard':
      return { title: `${prefix}플래시카드를 시작할 차례예요!`, sub: '단어 카드를 확인하면 다음 단계로 진행돼요' };
    case 'quiz':
      return { title: `${prefix}퀴즈를 시작할 차례예요!`, sub: '80점 이상이면 다음 단계로 진행돼요' };
    case 'spelling':
      return { title: `${prefix}스펠링을 시작할 차례예요!`, sub: '80점 이상이면 다음 단계로 진행돼요' };
    case 'matching':
      return { title: `${prefix}매칭을 시작할 차례예요!`, sub: '90점 이상이면 이 Day의 1회독이 완료돼요' };
    case 'r2_flashcard':
      return { title: `${prefix}2회독 플래시카드를 시작하세요!`, sub: '유의어·반의어·숙어를 학습해요' };
    case 'r2_quiz':
      return { title: `${prefix}종합 문제를 시작할 차례예요!`, sub: '80점 이상이면 다음 단계로 진행돼요' };
    case 'r2_matching':
      return { title: `${prefix}심화 매칭을 시작할 차례예요!`, sub: '90점 이상이면 이 Day의 2회독이 완료돼요' };
    default:
      return { title: `${prefix}${stage.label}을 시작하세요!`, sub: '' };
  }
}
