...
export async function fetchRawData(qc: SupabaseClient, sid: string, ninetyDaysAgo: string) {
  return Promise.all([
    ...
    // Topic accuracy: recent 500 problem attempts
    qc.from('naesin_problem_attempts').select('score, total_questions, created_at, unit_id, sheet_id').eq('student_id', sid).order('created_at', { ascending: false }).limit(500),
  ]);
}
